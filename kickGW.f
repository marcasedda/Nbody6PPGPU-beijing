...
!      STOP
      ...
      IF(kicktype .EQ. -4)THEN
         VKICK = 0.E0
      ELSE IF(kicktype .EQ. -3)THEN
         VKICK = 5.E0
      ELSE IF(kicktype .EQ. -2)THEN
         VKICK = 20.E0
...
****** adimensional spins:      SPIN(I1), SPIN(I2)         
         ...
         IF(ASPN(I1).LE.0.0.OR.ASPN(I1).GT.1.0)THEN     
            CALL SPIN_SELECT(0,-1,-1,number1)
            ASPN(I1) = number1
         ENDIF
         IF(ASPN(I2).LE.0.0.OR.ASPN(I2).GT.1.0)THEN
            CALL SPIN_SELECT(0,-1,-1,number2)
            ASPN(I2) = number2
         ENDIF   
...
*      CALL JPRED(I,TIME,TIME)
      DO 10 K = 1,3
          VI2 = VI2 + XDOT(K,III)**2
          XDOT(K,III) = XDOT(K,III) + VK(K)/VSTAR
          X0DOT(K,III) = XDOT(K,III)
          VF2 = VF2 + XDOT(K,III)**2
 10   CONTINUE
*     Modify energy loss due to increased velocity of single particle.
      DETMP = - 0.5*BODY(III)*(VF2 - VI2)

      III = I1            
      VI2 = 0.0
      VF2 = 0.0
      CALL JPRED(III,TIME,TIME)
      DO 11 K = 1,3
          VI2 = VI2 + XDOT(K,III)**2
          XDOT(K,III) = XDOT(K,III) + VK(K)/VSTAR
          X0DOT(K,III) = XDOT(K,III)
 ...
 11   CONTINUE
*     Modify energy loss due to increased velocity of single particle.
      DETMP = DETMP - 0.5*BODY(III)*(VF2 - VI2)
